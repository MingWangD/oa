...
- **核心成果**：
    ...
    - 已完成 `received-entrust`、`preliminary-survey`、`payment-notice`、`reject-acceptance`、`quality-control`、`field-survey`、`material-receive-return` 首轮高保真校准。
    - 最新完整验证：后端 `mvn test` 共 26 个测试通过，前端 `npm run build` 通过。
...
4. **高保真校准**：完成了 `material-receive-return` (材料接收与返还) 的高保真表单与流程配置，在 `JudicialConfigImportService` 中落地，并补充了测试。

## 🎯 下一步对齐焦点 (供 Gemini/Codex 接手)
- **优先任务**：继续第四阶段，校准 `draft-opinion-review / 鉴定意见书征求意见稿送审稿编制` 或 `final-opinion-review / 鉴定意见书送审稿编制`。
- **本次建议目标**：在 `JudicialConfigImportService` 中为意见书编制流程增加专门表单和流程配置，补对应的单元测试。
- **业务重点**：这是流程的核心产出环节，需确保意见书草稿、各级审核流向、盖章件回传以及最终归档的完整链路。
- **验证要求**：修改后运行后端 `mvn test` 和前端 `npm run build`。当前完整验证基线是后端 26 个测试通过，前端构建通过。
