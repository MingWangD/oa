...
## 📊 当前项目状态摘要 (2026-06-11)
- **阶段**：第三阶段底座已基本完成，第四阶段司法鉴定高保真流程进行中。
- **核心成果**：
    ...
    - 已完成 `received-entrust`、`preliminary-survey`、`payment-notice`、`reject-acceptance`、`quality-control`、`field-survey`、`material-receive-return`、`draft-opinion-review`、`final-opinion-review` 首轮高保真校准。
    - 最新完整验证：后端 `mvn test` 共 27 个测试通过，前端 `npm run build` 通过。
...
4. **高保真校准**：完成了 `material-receive-return` (材料接收与返还)、`draft-opinion-review` (鉴定意见书征求意见稿送审稿编制) 和 `final-opinion-review` (鉴定意见书送审稿编制) 的高保真表单与流程配置，在 `JudicialConfigImportService` 中落地，并补充了测试。

## 🎯 下一步对齐焦点 (供 Gemini/Codex 接手)
- **优先任务**：继续第四阶段，校准 `issue-opinion / 出具鉴定意见书` 或 `issue-draft-opinion / 出具征求意见稿`。
- **本次建议目标**：在 `JudicialConfigImportService` 中为该意见书出具流程增加专门表单和流程配置，补对应的单元测试。
- **业务重点**：确保出具环节与前序意见书编制环节平滑衔接，覆盖承诺书、复核意见、盖章、并行开票和归档材料。
- **验证要求**：修改后运行后端 `mvn test` 和前端 `npm run build`。当前完整验证基线是后端 27 个测试通过，前端构建通过。
...
- 第三阶段文件、知识库、审计、自动归档底座已完成主体能力；查重、水印、病毒扫描、全文检索仍是后续增强项。
- 第四阶段司法鉴定高保真流程进行中：20 个流程/19 个表单可导入，`received-entrust`、`preliminary-survey`、`payment-notice`、`reject-acceptance`、`quality-control`、`field-survey` 已完成首轮高保真配置。
- 尚未完成：20 个流程逐条真实案件运行验证、父流程回写细化、页面侧子流程展示、剩余流程高保真校准、完整 OA 其它业务域落地。
- 本地环境已经跑通：Java 17、MySQL 3307、Redis 6379、MinIO 9000、后端 8080、前端 5173。
- 最新验证结果：后端 `mvn test` 共 25 个测试通过，前端 `npm run build` 通过，`admin / Admin123` 登录成功。

### 协作约定

- Gemini 已提交的 RBAC、动态菜单和数据权限代码已由 Codex 延续加固，后续不回退该实现方向。
- 下一项司法鉴定开发应优先校准 `material-receive-return`，并开始做真实案件路径联调；公共平台增强项后续继续补查重、水印、病毒扫描、全文检索和成熟拖拽设计器。
