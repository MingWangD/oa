...
- **优先任务**：
  1. **表单数据持久化**：升级 `WorkflowRuntimeService`，确保办理任务时提交的 `formData` 能真实持久化到案件记录中。
  2. **网关与并行能力**：实现 `parallel` 拆分与 `inclusive` 汇聚的真实流转逻辑。
  3. **E2E 全链路联调**：用一个真实案件跑通从 `received-entrust` 到 `archive` 的全链路数据库级流转。
