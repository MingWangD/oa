...
- **已完成任务**：
  ...
  3. **子流程回写机制**：已解决子流程结束后的父流程唤醒回写能力（引入 `subflow_running` 状态占位，子流程 END 后自动触发完成操作恢复父干线流转）。
- **优先任务**：
  1. **E2E 全链路联调**：用一个真实案件跑通从 `received-entrust` 到 `archive` 的全链路数据库级流转。
- **验证要求**：所有引擎升级必须伴随单元测试，并跑通本地 `mvn test`。
