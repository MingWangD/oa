...
- 已完成首条司法鉴定流程高保真校准底座：
  - `received-entrust` 表单已从通用 JSON 样例升级为真实字段组、必填校验、附件查重、分支字段和自动归档元数据
  - `received-entrust` 流程已配置发起、收案员登记、部门负责人审阅、项目负责人决策、项目辅助人通知、材料接收、初步勘验、缴费、不予受理和结束节点
  - 流程运行时已支持基于 `formData` 的简单条件表达式，避免多条出边无条件全部触发
- 已完成子流程/关联流程最小运行模型：
  - 新增 `migration_v5_subflow_relation.sql`，补齐 `case_subflow_instance` 的父任务、父节点、子流程编码和名称字段
  - `WorkflowRuntimeService` 已支持根据 `transitionConfigJson` 自动创建子流程实例，并将后续节点/任务挂接到 `subflowInstanceId`
  - `received-entrust` 的初步勘验、发交费通知、不予受理三条路径已声明为自动关联发起子流程
- 已完成子流程结束语义与联调查询补强：
  - 子流程节点流转到 `END` 时不再误将整个案件主流程直接办结，而是优先回写到剩余活动任务
  - 新增 `/api/cases/{caseId}/subflows`，可查询案件已触发的全部子流程实例
  - 任务摘要、任务详情和前端 API 类型已补充 `subflowInstanceId`
- 已切换为真实子流程节点执行：
  - 命中 `launchSubflow` 后，会直接根据子流程定义选择首个可办节点创建任务
  - 挂接 `subflowInstanceId` 的任务后续办理时，会按子流程自己的 `wf_definition / wf_transition_def` 继续流转
- 已修复后端编译与测试所需的缺失能力
- 后端 `mvn test` 已通过
- 已新增权限、设计器、custom 数据权限、运行时动态流转、司法鉴定配置导入、自动归档、条件流转、子流程关联和父流程回写测试，当前后端共 20 个测试通过
...
- 第四阶段：逐条跑通司法鉴定细化流程，继续落实子流程、流程关联、各节点真实产物和路径验收

## 下一阶段重点

1. 对本次新增 `migration_v4_file_knowledge_audit.sql` 执行数据库迁移
2. 在平台总览页执行“导入平台配置”，发布 20 个司法鉴定流程和 19 个表单
3. 按司法鉴定核对矩阵，从“收到委托书”开始跑通初步勘验、发交费、不予受理三条真实路径，并继续补节点级父流程回写细节
