...
合同管理当前应判定为**有条件通过的第五阶段 MVP 启动项**，不是完整合同系统完成。

已符合第五阶段第一条最小闭环的部分：

- 合同创建：已有真实主表 `contract_info`，前端可新建合同。
- 合同版本：已有 `contract_version` 保存合同内容快照。
- 合同附件：已有 `contract_attachment`，可上传或绑定已有文件。
- 合同审批：已有轻量状态流转，覆盖提交、部门审核通过、驳回。
- 知识库归档：审批通过后归档到“合同档案”目录。
- 审计留痕：创建、修改、提交、审批、归档、驳回均写入审计事件。
- 前端查询：`/placeholder/contract` 已可查询列表、打开详情并执行动作。

仍不满足《完整 OA 系统重构需求规格说明书》中完整合同管理要求的部分：

- 合同评审、签署、生效、履行、完成、终止仍未形成完整状态链。
- 收付款计划、实际收款、开票联动、到期提醒、统计分析未实现。
- 合同变更、续签、终止、归档后的版本对比和完整审批历史仍未补齐。
- 金额分级审批、跨部门会签、总经理审批等复杂流程尚未接入动态工作流引擎。
- 合同数据权限当前由 Service 层按“本人 / 本部门 / ADMIN”控制，后续应纳入统一行级数据权限策略。
- 目前已有合同服务单元测试，但仍缺一条真实数据库级合同长链路集成测试，用于断言归档文档、审计事件和权限边界。

因此，后续评审时不要只看 `mvn test` 和前端构建通过，还要继续检查合同闭环集成测试、权限测试、归档断言和审计断言。合同管理可以继续推进，但不能替代第四阶段司法鉴定 20 流程 / 19 表单的使用手册级逐条验收。
...
- `migration_v6_form_data.sql`
- `migration_v7_contract_mvp.sql`
...
- 合同管理当前只是第五阶段 MVP：已跑通创建、版本、附件、提交、部门审核、归档和审计，但完整评审、签署、收付款、提醒、统计、变更、终止仍未完成
...
- 同时继续推进第五阶段合同管理纵深开发，优先补合同闭环集成测试、权限边界、审计/归档断言、审批历史、付款计划只读展示，再逐步推进 `仓库 / 风险 / 行政 / 人资 / 考勤 / 公文 / 交流 / 督查 / 报表 / 集成` 等模块从结构化看板升级到真实业务表、审批动作和详情页
