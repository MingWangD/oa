...
1. **已补齐运行态表单数据持久化**：确保引擎在流程扭转时，能真实存取并计算 `formData`。
2. **已打通真实引擎底座**：已补齐真实并行（Parallel）/汇聚（Inclusive）网关的执行能力与多分支任务等待逻辑；已补齐真实子流程挂起与结束回写唤醒父流程的能力。
3. **真实案件 E2E 跑通长链路**：从 `received-entrust` 发起，走通 `material` -> `draft` -> `issue` -> 到归档的真实数据库链路。
4. **升级验收矩阵**：把 20 流程自动验收升级为“真实场景验证通过”。
5. **第五阶段业务闭环**：完整 OA 不做占位符，从 CRM/合同/项目中选一个模块做成真正业务闭环（建表、权限、数据隔离）。

## 给项目组的当前交接

当前处于第四阶段的“深水区”：**运行时引擎能力攻坚**。

下一条优先任务**不再是**配置新的流程，而是：
**用真实案件联调跑通 E2E 长链路，在前端界面将表单与流程流转串联起来。**
