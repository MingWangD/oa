...
@SpringBootTest
@Transactional
class DraftOpinionReviewBranchVerificationTest {
    ...
    @Test
    void qualityControlDecision_shouldLaunchDraftOpinionReviewToProjectLeaderFirst() {
        Long caseId = createCaseAtDraftOpinionReviewFromQuality("9.7-从质控进入征求意见稿送审稿编制");

        assertThat(activeTaskNodeCodes(caseId)).contains("DRAFT_OPINION_REVIEW", "PROJECT_ASSIGN");
        assertThat(activeTaskNodeCodes(caseId)).doesNotContain("ASSISTANT_DRAFT");
        assertThat(runningSubflowCodes(caseId)).contains("draft-opinion-review");
        assertThat(caseInfoMapper.selectById(caseId).getFormData())
                .containsEntry("nextRecommendation", "鉴定意见书征求意见稿送审稿编制");
    }

    @Test
    void fieldSurveyDecision_shouldLaunchDraftOpinionReviewSubflow() {
        Long caseId = createCaseAtFieldSurveyNextDecision("9.7-从现场勘验进入征求意见稿送审稿编制");

        completeTask(caseId, "NEXT_FLOW_DECISION", ActionCode.APPROVE,
                Map.of("nextRecommendation", "鉴定意见书征求意见稿送审稿编制"), null);
    ...
    @Test
    void materialReceiveReturnDecision_shouldLaunchDraftOpinionReviewSubflow() {
        Long caseId = createCaseAtMaterialProjectDecision("9.7-从材料接收与返还进入征求意见稿送审稿编制");
    ...
    @Test
    void happyPath_shouldKeepThreeLevelSerialReviewAndLaunchIssueDraftOpinion() {
        Long caseId = createCaseAtDraftOpinionReviewFromQuality("9.7-三级审核通过进入出具征求意见稿");
        ...
        completeTask(caseId, "PROJECT_REVIEW", ActionCode.APPROVE,
                Map.of("projectReviewPassed", true), null);
    ...
    void projectReviewReturn_shouldReturnToAssistantDraft() {
        Long caseId = createCaseAtDraftProjectReview("9.7-项目负责人审核退回");
    ...
    void technicalReviewReturn_shouldReturnToProjectReview() {
        Long caseId = createCaseAtDraftTechnicalReview("9.7-技术负责人审核退回");
    ...
    void departmentReviewReturn_shouldReturnToTechnicalReview() {
        Long caseId = createCaseAtDraftDepartmentReview("9.7-部门负责人审核退回");
    ...
    void projectFinalUploadReturn_shouldReturnToDepartmentReview() {
        Long caseId = createCaseAtDraftFinalUpload("9.7-项目负责人定稿退回");

        completeTask(caseId, "PROJECT_FINAL_UPLOAD", ActionCode.RETURN,
                Map.of("finalDraftUploaded", false), "定稿版本需补充");

        assertThat(activeTaskNodeCodes(caseId)).contains("DEPARTMENT_REVIEW");
        assertThat(activeTaskNodeCodes(caseId)).doesNotContain("ISSUE_DRAFT_OPINION");
    }
    ...
    private Long createCaseAtDraftProjectReview(String title) {
        Long caseId = createCaseAtDraftOpinionReviewFromQuality(title);
        completeTask(caseId, "PROJECT_ASSIGN", ActionCode.APPROVE, Map.of(), null);
        completeTask(caseId, "ASSISTANT_DRAFT", ActionCode.APPROVE,
                draftOpinionFormData(), null);
        assertThat(activeTaskNodeCodes(caseId)).contains("PROJECT_REVIEW");
    ...
    private Long createCaseAtDraftOpinionReviewFromQuality(String title) {
    ...
    private Long createSubmittedReceivedEntrustCase(String title) {
        ...
        caseInfoService.submitCase(caseInfo.getId(), new CaseSubmitRequest(OPERATOR_ID, OPERATOR_NAME, "发起 9.7 分支验证"), OPERATOR_ID, OPERATOR_NAME);
        return caseInfo.getId();
    }

    private Map<String, Object> receivedEntrustFormData() {
        Map<String, Object> data = new java.util.HashMap<>();
        data.put("clientName", "9.7 分支验证委托人");
        data.put("serialNo", "BRANCH-9-7");
        data.put("initiatorName", "测试发起人");
        data.put("initiatedDate", "2026-06-13");
        data.put("receivedDate", "2026-06-13");
        data.put("caseNo", "JA-BRANCH-9-7");
        data.put("appraisalCategory", "工程造价");
        data.put("urgencyLevel", "普通");
        data.put("caseChannel", "线下");
        data.put("appraisalMatter", "9.7 鉴定意见书征求意见稿送审稿编制分支验证");
    ...
    private Map<String, Object> qualityControlFormData() {
        ...
        data.put("qualityFileSummary", "征求意见稿送审稿编制前质量控制文件摘要。");
        ...
        data.put("nextRecommendation", "鉴定意见书征求意见稿送审稿编制");
        return data;
    }
    ...
    private Map<String, Object> draftOpinionFormData() {
        Map<String, Object> data = new java.util.HashMap<>();
        data.put("caseNo", "JA-BRANCH-9-7");
        data.put("projectLeaderId", 3L);
        data.put("projectAssistantId", 4L);
        data.put("technicalLeaderId", 5L);
        data.put("departmentHeadId", 2L);
        data.put("draftOpinionUploaded", true);
        data.put("projectReviewPassed", true);
        data.put("technicalReviewPassed", true);
        data.put("departmentReviewPassed", true);
        data.put("finalDraftUploaded", true);
        data.put("nextRecommendation", "出具征求意见稿");
        return data;
    }
    ...
    private void completeTask(Long caseId, String nodeCode, ActionCode actionCode, Map<String, Object> formData,
        ...
        WorkflowActionRequest request = new WorkflowActionRequest(
                task.getId(), actionCode, "9.7 分支验证", reason, null, null,
