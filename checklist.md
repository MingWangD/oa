...
| 21 | 9.15 | 退费流程 | [x] | `RefundBranchVerificationTest` 已覆盖合同变更、收入确认、退费申请、财务打款及进入终止鉴定路径 |
| 22 | 9.16 | 财务报销 | [ ] | `finance` 账号新建工作可见入口已验证；独立流程运行级测试与页面验收待补 |
| 23 | 9.17 | 终止鉴定 | [x] | `TerminateAppraisalBranchVerificationTest` 已覆盖初勘不具备条件、未缴费、撤案无需退费 3 类上游触发，终止文书编制/审核、用章子流程等待与唤醒、盖章件回传、退回路径、进入归档 |
| 24 | 9.18 | 归档 | [x] | `ArchiveBranchVerificationTest` 已覆盖整理、邮寄分支、中心审核退回、正式入库及终态 |
| 25 | 9.19 | 用章流程/用章申请表 | [x] | `SealApplicationBranchVerificationTest` 已覆盖独立发起、子流程触发等待/唤醒、审核退回全路径 |

## 下一步顺序

| 优先级 | 任务 | 状态 |
|---:|---|---|
| 1 | 补 9.15 退费流程完整运行级测试 | [x] |
| 2 | 补 9.17 终止鉴定完整运行级测试 | [x] |
| 3 | 补 9.18 归档完整运行级测试和知识库页面验收 | [x] |
| 4 | 补 9.19 用章申请表独立发起与父流程等待专项验收 | [x] |
| 5 | 补 9.16 财务报销独立流程 | [ ] |
